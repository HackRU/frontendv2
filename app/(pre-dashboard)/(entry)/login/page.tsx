    ...
    <main className="flex items-center justify-center md:h-screen w-screen h-screen">
      <form onSubmit={handleSubmit(onSubmit)} className='bg-gradient-to-b from-[var(--bg-color)] to-[var(--bg-color2)] p-20 rounded-xl'>
        <div className="w-full grid gap-0 items-center">
          ...
          <div>
          <p className = "text-s italic  text-[var(--mainText-color)]">Press Login or Press Enter to Login</p>
            <label
          ...
          <p className="text-s italic  text-[var(--mainText-color)] mt-2 hover:text-[var(--hover-color)] cursor-pointer" onClick={() => router.push('/signup')}>Not a member? Create an Account!</p>
          <p className="text-s italic  text-[var(--mainText-color)] mt-2 hover:text-[var(--hover-color)]" onClick={() => router.push('/forgot')}>Forgot Password? Reset it Here!</p>
        </div>
      </form>
    </main>
  );
